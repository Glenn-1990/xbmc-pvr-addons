...
{
  /* Not supported */
  //if (m_conn.GetProtocol() < 12) temp workaround
	tvherror("EDL NOT SUPPORTED");
...
PVR_ERROR CTvheadend::GetTimers ( ADDON_HANDLE handle )
{
  ...
  for (rit = m_recordings.begin(); rit != m_recordings.end(); ++rit)
  {
    ...
    tmr.bIsRepeating      = 0; //rit->second.autorec; workaround for gotham bug
...
PVR_ERROR CTvheadend::AddTimer ( const PVR_TIMER &timer )
{
  ...
  if (autorec)
  {
    ...
    {
      CLockObject lock(m_conn.Mutex());
      m = m_conn.SendAndWait("addDvrAutorecEntry", m);
    }
