...
PVR_ERROR CTvheadend::AddTimer ( const PVR_TIMER &timer )
{
  ...
  if (timer.startTime != 0 && timer.iEpgUid >= 0 && m_conn.GetProtocol() == 130)
  {
  ...
  if (autorec)
  {
    ...
    /* Send and Wait */
    if ((m = m_conn.SendAndWait("addDvrAutorecEntry", m)) == NULL)
    {
  ...
  ( const PVR_TIMER &timer, bool _unused(force) )
{
  ...
  if (timer.bIsRepeating && m_conn.GetProtocol() == 130)
  {
    CLockObject lock(m_conn.Mutex());
